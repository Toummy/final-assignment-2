...
entrada até a saída. Existem quatro tipos diferentes de salas possíveis,
identificadas por um número de 0 a 3:

\begin{itemize}
	\item \textbf{0}: não faz parte do caminho solução.
	\item \textbf{1}: contém saídas na esquerda e na direita. 
	\item \textbf{2}: contém saídas na esquerda, na direita e embaixo.
	\item \textbf{3}: contém saídas na esquerda, na direita e em cima.
\end{itemize}

Primeiramente, o gerador escolhe uma das quatro salas da parte superior para
colocar a entrada do nível. Escolhido o local da entrada, o algoritmo gera um
caminho até a saída através de movimentos aleatórios na grade de salas. Para
tal, seleciona um número aletório de 1 a 5. Caso o resultado seja 1 ou 2, o
gerador coloca uma sala à esquerda. Se o resultado for 3 ou 4, o gerador coloca
uma sala à direita. Por fim, se o resultado for 5, o gerador coloca uma sala
abaixo da posição atual. Caso o gerador atinja um dos extremos horizontais da
grade, ele coloca uma sala abaixo e inverte a direção horizontal do movimento
anterior.

Sempre que uma nova sala é colocada no caminho solução, ela pertence ao tipo 1
de salas. Assim, se o gerador se movimentar horizontalmente, tem-se a garantia
de saídas na esquerda e na direita. Contudo, se o gerador desejar se mover para
baixo, ele primeiramente deve modificar o tipo da sala atual de 1 para 2,
garantindo uma saída na parte inferior que abrirá passagem para a sala abaixo.
Logo após esta modificação, a próxima sala será sempre dos tipos 2 ou 3, pois
somente esses tipos de sala garantem que a conexão vertical se dará da maneira
correta, além de possuirem saídas horizontais, permitindo a continuidade do
algoritmo. Caso o algoritmo se encontre na última linha da grade e queira
colocar uma sala para baixo, ele seleciona aquela sala para conter a saída do
nível. Assim, a saída sempre se encontrará em uma das quatro salas da parte
inferior do mapa.
...
parte do caminho solução não possuem esta garantia, podendo ser completamente
seladas e somente acessíveis utilizando algum equipamento ou destruindo parte
do terreno. A Figura \ref{fig:spelunky-procgen-path} exemplifica um resultado
desta etapa do algoritmo.
...
primeira etapa do algoritmo de geração procedural de níveis de Spelunky. Os
\textbf{X} representam a entrada (parte superior) e a saída (parte inferior).}
\end{figure}
...
nível. Para fazer isto, o gerador se baseia em \textbf{modelos} de
\textit{layouts} de salas pré-estabelecidos pelo desenvolvedor, sendo que cada
tipo possui em torno de 6 a 12 modelos. Estes modelos são descritos através de
arquivos de texto comuns. A Figura \ref{fig:spelunky-procgen-room-template}
ilustra um exemplo de modelo utilizado pelo gerador.
...
Como pode ser observado na Figura \ref{fig:spelunky-procgen-room-template}, são
utilizados caracteres comuns para realizar a descrição do \textit{layout} de uma
sala. Cada caractere possui um significado diferente, e informam ao gerador que
tipo de \textit{tile}\footnote{Tipo de representação visual muito utilizada por
jogos 2D para dispor os elementos de um jogo em forma de grade, onde cada
elemento se chama \textit{tile}.} deve ser utilizado em sua posição. Uma sala em
Spelunky é representada por uma grade 10 por 8 de \textit{tiles}, o que
significa que cada arquivo de modelo deve conter exatamente 80 caracteres. A
Tabela \ref{tab:spelunky-procgen-rooms-tiles} lista os caracteres que podem ser
utilizados para realizar a construção de um modelo, bem como suas descrições.
Alguns caracteres são \textbf{estáticos}, ou seja, sempre que aparecem, serão
substituídos por seu \textit{tile} correspondente. Contudo, alguns caracteres
são \textbf{probabilísticos}, o que significa que, quando aparecem, existe uma
chance de serem substituídos por seu \textit{tile} correspondente ou serem
substituídos por um espaço vazio.

\begin{table}[htb]
\begin{center}
\caption{\label{tab:spelunky-procgen-rooms-tiles}Lista de caracteres que podem
ser utilizados para descrever um modelo de sala de Spelunky.}
\begin{tabular}{| c | c | c |}
\hline
\textbf{Caractere} & \textbf{Descrição} \\
\hline
0 & Espaço vazio \\
1 & Espaço sólido \\
L & Escada \\
P & Escada com plataforma auxiliar \\
5 & Conjunto aéreo \\
6 & Conjunto terrestre \\
2 & Espaço sólido (50\% de chance) \\
4 & Bloco de empurrar (25\% de chance) \\
7 & Armadilha de espinhos (33\% de chance) \\
\hline
\end{tabular}
\end{center}
\end{table}

Os conjuntos aéreo e terrestre são um caso especial. Eles não são mapeados para
um único \textit{tile}, e sim para um conjunto 5 por 3 de \textit{tiles}. Estes
conjuntos também utilizam modelos pré-definidos, descritos através de um arquivo
de texto simples, utilizando os mesmos caracteres apresentados na Tabela
\ref{tab:spelunky-procgen-rooms-tiles} -- com exceção dos caracteres "5" e "6".
A diferença entre os tipos de conjunto é simples: um conjunto terrestre deve ser
colocado próximo ao chão e um conjunto aéreo deve ser colocado no ar. 

Com a descrição dos caracteres em mão, podemos realizar uma interpretação do
modelo ilustrado na Figura \ref{fig:spelunky-procgen-room-template}. O modelo
descreve uma sala com chão, uma parede à direita e uma escada que leva a uma
pequena passagem, obstruída por um bloco de empurrar. 
