...
redes neurais e algoritmos genéticos, cujo objetivo era navegar da entrada até a
saída de cenários de testes estipulados. Para tal, o autor utilizou o
\textit{framework} de Inteligência Artificial \textit{SpelunkBots}.

A rede neural utilizada era uma simples rede de retroalimentação
(\textit{feed-forward}) de topologia fixa com as seguintes características:

\begin{itemize}
	\item \textbf{Entrada:} A rede possuía 4 nodos de entrada: distância do
	...
	\item \textbf{Conexões:} Os nodos de uma camada eram conectados
		todos-para-todos com os nodos da camada seguinte.

	\item \textbf{Pesos:} Os pesos das conexões apresentavam valores entre -1 e
		1.

	\item \textbf{Função de Ativação:} A função de ativação utilizada era do
		tipo sigmóide, mais especificamente uma modificação da função
		\textit{tanh}: $((e^{x} - e^{-x})/2) / ((e^{x} + e^{-x})/2)$. Esta função
		normaliza os valores, restringindo-os entre -1 e 1.
\end{itemize}

O algoritmo genético escolhido pelo autor era o de \textit{tournament
selection}, que ordena a população de organismos por aptidão e, depois, realiza
mutações e cruzamentos através de probabilidades (60\% de chance de mutação,
30\% de chance de cruzamento). A função de aptidão escolhida para mensurar a
qualidade dos agentes era baseada na distância do agente até a saída e o tempo
necessário para completar o nível. Os valores de distância variavam de 1 a 100 e
os de tempo de 0 a 1 (normalização entre o tempo necessário e o tempo máximo de
teste). Quanto mais próximo da saída, mais alta a pontuação e quanto menos
tempo, menos a penalização. Caso o agente atingisse o fim do mapa, ele receberia
a pontuação máxima de distância.

Para se comunicar com o jogo, utilizou o \textit{SpelunkBots}. Assim, recebia os
valores necessários para suas entradas da rede e enviava os comandos de acordo
com as saídas da rede. Com o objetivo de testar a eficácia das técnicas
escolhidas, o autor desenvolveu 10 cenários de teste, executou os agentes em
cada um deles e coletou resultados destas execuções para análise. É importante
ressaltar que os cenários escolhidos pelo autor eram extremamente simples e não
correspondem com um mapa gerado proceduralmente pelo jogo.
