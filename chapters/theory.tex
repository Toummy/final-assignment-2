...
Estes canais possuem um \textbf{peso} atrelado a eles, que informam à unidade o
...
As redes sem realimentação são mais simples, mas as recorrentes permitem a
aquisição de memória de curto-prazo. Por fins de simplificação, a explicação
desta seção se concentrará em redes sem realimentação.
...
escondidas é chamada de \textbf{multi-camada}. Muitas vezes, não é necessário
adicionar camadas escondidas, mas existem funções que não podem ser aproximadas
corretamente com apenas uma camada (a função lógica \textit{XOR}, por exemplo)
por causa do problema de separabilidade linear (detalhes em \cite[Cap.
18]{RussellNorvig200912}). As unidades de processamento são conectadas
(geralmente, todos os neurônios de uma camada possuem conexão com todos os
neurônios da camada seguinte) e a rede neural se forma. Dependendo do problema,
a escolha de quantas camadas e unidades de processamento afeta a qualidade dos
resultados obtidos. Não existe uma ciência exata para definir quantidades de
camadas e neurônios, ficando a critério do desenvolvedor a análise do problema e
a experimentação de topologias \cite[Cap. 18]{RussellNorvig200912}. A Figura
\ref{fig:neural-networks-example} ilustra uma possível topologia de uma rede
neural artificial sem realimentação.
